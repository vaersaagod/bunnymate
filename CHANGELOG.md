...
- Signed playback URLs are now minted as the response is prepared, rather than when the template renders, so a token can't be baked into a `{% cache %}` block and outlive itself. Substitution survives `json_encode` and HTML escaping, so URLs inside a `data-sources` attribute are covered too.
